...
void tcpClient::handleReleaseSession(string message)
{
    ...
    if (sessions.find(loco) != sessions.end())
    {
        ...
        logger->debug("[%d] [tcpClient] Loco %d not allocated", id, loco);
    // inform the ED
    if (special_release)
    {
        i = message.find("<;>");
...
void tcpClient::handleSpeed(string message)
{
    ...
    else
    {
        ...
        edspeed = speed;
        if (speed == 1)
        { // can't send speed 1 (emergency stop) to cancmd
            speed++;
        }
    }
...
void tcpClient::shutdown()
{
    ...
    string command = "systemctl stop canpid";
...
void tcpClient::sendFnMessages(edSession *session, int fn, string message)
{
    ...
    if (session->getFnState(fn) == FN_STATE::ON)
        session->setFnState(fn, FN_STATE::OFF);
    else
        session->setFnState(fn, FN_STATE::ON);
