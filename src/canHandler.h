...
public:
    ...
    bool isAutoEnumMode() const { return auto_enum_mode; }
    std::queue<frameCAN> &getOutMsgs() { return out_msgs; }
    void doSelfEnum();
...
private:
    ...
    // Helper method to put a frame in the out_msgs queue
    void putToOutQueue(const frameCAN &frame)
    {
        out_msgs.push(frame);
    }
};
